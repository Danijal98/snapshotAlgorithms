...
public class SnapshotCollectorWorker implements SnapshotCollector {
	...
	public void run() {
		while(working) {
			...
			case NAIVE:
				...
				Message askMessage = new NaiveAskAmountMessage(AppConfig.myServentInfo, AppConfig.myServentInfo, CausalBroadcastMessage.mapDeepCopy(myClock));

				for (Integer neighbor : AppConfig.myServentInfo.getNeighbors()) {
					Message msg = askMessage.changeReceiver(neighbor);
					MessageUtil.sendMessage(msg);
				}

				CausalBroadcastShared.addPendingMessage(askMessage);
	...
	public void addNaiveSnapshotInfo(String snapshotSubject, int amount) {
		...
		System.out.println(collectedNaiveValues);
	}
